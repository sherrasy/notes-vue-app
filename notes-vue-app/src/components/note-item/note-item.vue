...
<template>
    <div class="note__container" :class="{'task-done':props.note.isDone, 'edit-active':isEditing}">
        ...
        <div class="note__controls">
            <button class="note__button note-btn-edit" @click="handleEditing"> {{ isEditing ? 'Отменить ввод' : 'Редактировать' }}</button>
            <button class="note__button note-btn-delete" @click="handleDelete"> Удалить</button>
            <button class="note__button note-btn-done" @click="handleUpdateDone"> {{ props.note.isDone ? 'Не готово' : 'Выполнено' }}</button>
        </div>
    </div>
</template>